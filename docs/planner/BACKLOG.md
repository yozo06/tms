...
> Last updated: 2026-04-06 by Planner — H-17 (RLS SQL), H-18 (aria-label fix), H-19 (zone_summary SQL) completed. All three batched in one session.
>
...
|---|------|-------|--------|--------|-------|
| C-03 | **Add rate limiting middleware** | 90 | S1 | ✅ Done | Custom zero-dependency in-memory sliding-window rate limiter. Auth: 5 failed attempts/15 min (skipSuccessfulRequests). Signup: 3/hour. Global API: 200/min. Includes Retry-After + X-RateLimit headers. Suitable for single-instance; upgrade to Redis for multi-instance. |
| C-04 | **Add input sanitization middleware** | 88 | S1 | ✅ Done | Custom zero-dependency XSS sanitizer. HTML entity escaping on all string inputs (body, query, params). Null byte stripping, whitespace trimming, max-length enforcement. Password/token fields excluded. Applied globally after body parsing. |
...
| H-08 | **Expand test coverage: auth API routes** | 76 | S2 | ✅ Done | 14 Supertest tests in `src/modules/auth/routes/auth.test.ts`. Covers: login (7 cases: missing fields, user not found, deactivated, wrong pw, success, email normalisation), signup (5 cases: missing fields, email in use, DB failure, success), refresh (3 cases: missing token, invalid/expired, success). Uses vi.mock() for Supabase + auth lib. Tests runnable after `npm install`. 2026-03-29. |
...
| H-16 | **Backend API query scoping — filter all endpoints by project_id** | 83 | S2 | ✅ Done | requireProject middleware (src/core/middleware/requireProject.ts): validates X-Project-Id header, sets req.projectId. GET /api/projects endpoint added (src/modules/auth/routes/projects.ts). Trees, zones, dashboard, map all filter by project_id when header present. Frontend client.ts sends X-Project-Id from Zustand persisted state. Committed b1d14c3. 2026-03-30. |
...
| H-17 | **Write RLS policy SQL (005_rls_policies.sql)** | 78 | S3 | ✅ Done | `docs/migrations/005_rls_policies.sql`: RLS enabled on 8 tables (trees, land_zones, activity_log, health_observations, tree_photos, tree_contributors, projects, project_members). Custom `current_app_user_id()` + `user_is_project_member()` + `user_has_project_role()` helper functions. Role-based CRUD policies (viewer→admin hierarchy). 6 performance indexes. Full rollback script included. ⚠️ Yogesh needs to run in Supabase SQL Editor. |
| H-18 | **Fix dismiss button aria-labels (MapView + TreeDetail)** | 70 | S2 | ✅ Done | Added `aria-label="Dismiss selection"` to MapView ✕ button. Added `aria-label="Go back"` and `aria-label="Edit tree"` to TreeDetail nav buttons. Fixed in legacy `pages/` files (modular copies already had labels). |
| H-19 | **Update zone_summary DB view to include project_id** | 68 | S3 | ✅ Done | `docs/migrations/008_zone_summary_project_id.sql`: Drops and recreates zone_summary view with project_id from land_zones. Dashboard route updated to filter by project_id. ⚠️ Yogesh needs to run in Supabase SQL Editor. |
...
|---|------|-----------|---------|
| C-03 | Rate limiting: custom in-memory sliding-window middleware in `src/core/middleware/rateLimit.ts`. Auth (5 failed/15min), signup (3/hour), global API (200/min). Applied to auth routes + global /api. | 2026-03-31 | Planner 2026-03-31 |
| C-04 | Input sanitization: custom XSS prevention middleware in `src/core/middleware/sanitize.ts`. HTML entity escaping, null byte stripping, trim, max-length. Applied globally after body parsing. Excludes passwords/tokens. | 2026-03-31 | Planner 2026-03-31 |
| H-16 | Project scoping: requireProject middleware + GET /api/projects + X-Project-Id header in client. All arbor routes filter by project_id when active. Frontend sends project header via axios interceptor. Commit b1d14c3. | 2026-03-30 | Planner 2026-03-30 |
| H-08 | Auth API route tests: 14 Supertest cases in `src/modules/auth/routes/auth.test.ts` covering login (7 cases: missing fields, user not found, deactivated, wrong pw, success, email normalisation), signup (5), refresh (3). vi.mock() for Supabase + auth lib. | 2026-03-29 | Planner 2026-03-29 |
...
| H-17 | Write RLS policy SQL (005_rls_policies.sql): 8 tables, role-based CRUD, helper functions, indexes | 2026-04-06 | Planner 2026-04-06 |
| H-18 | Fix dismiss button aria-labels in MapView + TreeDetail legacy pages | 2026-04-06 | Planner 2026-04-06 |
| H-19 | Update zone_summary view SQL (008_zone_summary_project_id.sql) + dashboard route update | 2026-04-06 | Planner 2026-04-06 |
