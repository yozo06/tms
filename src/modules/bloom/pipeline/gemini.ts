  ...
  async generateCarousel(opts: {
    ...
    for (let i = 0; i < opts.slidePrompts.length; i++) {
      ...
      try {
        ...
        console.error(`  ⚠️ Slide ${i + 1} failed: ${errMsg}`);

        // Detect billing/free-tier errors — abort the whole carousel, retrying won't help
        const isFreeTier = errMsg.includes('FreeTier') ||
          errMsg.includes('"limit": 0') || errMsg.includes('"limit":0') ||
          errMsg.includes('free_tier_requests') || errMsg.includes('free_tier_input_token');
        if (isFreeTier) {
          console.error(`\n  💳 BILLING REQUIRED — aborting carousel. Enable billing at: https://aistudio.google.com/plan\n`);
          break; // Stop generating remaining slides
        }
        // Continue with remaining slides for other errors
      }
    }
