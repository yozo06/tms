 ...
 *
 * Usage:
 *   npx ts-node src/modules/bloom/pipeline/cli.ts <command> [options]
 *
 * Commands:
 ...
 * Examples:
 *   # List all templates
 *   npx ts-node src/modules/bloom/pipeline/cli.ts list
 *
 ...
 *     --ref content/bloom/references/watercolor-botanical/bamboo-architecture.png
 *
...
async function cmdList(options: Record<string, string>) {
  ...
  console.log(`\n🌸 WildArc Bloom — Content Templates (${templates.length})\n`);
  ...
  console.log('\n' + '━'.repeat(70));
  console.log(`\nUsage: npx ts-node src/modules/bloom/pipeline/cli.ts generate --template <id> --topic "..."\n`);
}
...
function cmdHelp() {
  console.log(`
🌸 WildArc Bloom — Content Generation Pipeline
...
Quick start:
  npx ts-node src/modules/bloom/pipeline/cli.ts list
  npx ts-node src/modules/bloom/pipeline/cli.ts generate --template infographic-educational --topic "Bamboo propagation methods"

Reference images:
  Drop style references into content/bloom/references/<style-preset>/
  Templates auto-discover references matching their style.
`);
}
